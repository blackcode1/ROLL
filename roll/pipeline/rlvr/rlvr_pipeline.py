...
class RLVRPipeline(BasePipeline):
    ...
    def __init__(self, pipeline_config: RLVRConfig):
        ...
        if self.pipeline_config.use_reference and not self.is_lora:
            ...
            with tps_timer, Timer(name="step_total", logger=None) as step_total_timer:
                ...
                with Timer(name="cal_ref_log_probs", logger=None) as cal_ref_log_probs_timer:
                    if self.pipeline_config.use_reference:
                        ...
                        # When not using reference model, create empty ref_log_probs with zeros
                        batch.batch["ref_log_probs"] = torch.zeros_like(batch.batch["responses"], dtype=torch.float32)
