...
class AgenticPipeline(BasePipeline):
    def __init__(self, pipeline_config: AgenticConfig):
        ...
        if not self.pipeline_config.disable_reference_model:
            ...
            with tps_timer:
                ...
                with Timer(name="cal_ref_log_probs", logger=None) as cal_timer:
                    if not self.pipeline_config.disable_reference_model:
                        ...
                        # When reference model is disabled, use actor_train log probs as reference
    ...
    def adjust_batch(self, data: DataProto, mode="copy") -> DataProto:
        ...
        ref_infer_bsz = self.pipeline_config.reference.infer_batch_size * self.reference.dp_size if not self.pipeline_config.disable_reference_model else actor_train_infer_bsz
        critic_train_bsz = 1
        critic_infer_bsz = 1
        if self.pipeline_config.adv_estimator == "gae":
