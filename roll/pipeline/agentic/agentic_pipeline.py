...
class AgenticPipeline(BasePipeline):
        ...
        for global_step in range(self.pipeline_config.max_steps):
            ...
            with tps_timer:
                ...
                if self.pipeline_config.enable_reference:
                    ...
                    # Use actor_train as reference when reference model is disabled
                    with Timer(name="cal_ref_log_probs", logger=None) as cal_timer:
    ...
    def adjust_batch(self, data: DataProto, mode="copy") -> DataProto:
        ...
        if self.pipeline_config.enable_reference:
            ...
            ref_infer_bsz = actor_train_infer_bsz  # Use actor_train batch size when reference is disabled
        critic_train_bsz = 1
        critic_infer_bsz = 1
        if self.pipeline_config.adv_estimator == "gae":
