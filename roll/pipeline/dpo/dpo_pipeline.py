                ...
                with Timer(name="step_total", logger=None) as step_total_timer:
                    ...
                    with Timer(name="cal_ref_log_probs", logger=None) as cal_ref_log_probs_timer:
                        if self.pipeline_config.use_reference:
                            ...
                            # When not using reference model, create empty reference_log_probs with zeros
                            import torch
                            batch_size = batch.batch.batch_size[0]
                            dummy_ref_log_probs = torch.zeros(batch_size, dtype=torch.float32)
                            batch.batch["reference_log_probs"] = dummy_ref_log_probs
